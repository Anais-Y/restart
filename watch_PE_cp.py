...
for epoch in tqdm.tqdm(range(num_epochs)):
    ...
    for name, param in model.named_parameters():
        if param.grad is not None:
            ...
            if not (np.isnan(grad).any() or np.isinf(grad).any() or np.isnan(param.data.cpu().numpy()).any()):
                ...
                print(f"Warning: Gradient for {name} contains NaN or Inf, skipping logging.")
                state = {'state_dict': model.state_dict(), 'hyperparams': vars(args)}
                torch.save(state,
    ...
    wandb.log({
        ...
        # y_true=te_labels,
        # preds=te_preds,
        # class_names=["0", "1", "2"] if args.dataset == "SEED" else ["0", "1", "2", "3"]) 
    })
...
final_info =f'Test Acc: {te_acc_max}, Confusion Matrix: {cm}'
