...
def load_data_label(file_path):  # , how='valence'
    ...
    new_label = dat['labels'][:, 0:2]  # get valence and arousal
...
def compute_features_96(filtered_data):  # , baseline  num_windows: 每s要分成多少个windows
    data = filtered_data[:, :, 3 * 128:, :].transpose((0, 2, 1, 3))  # 40 7680 32 4
    videos, t, chans, bands = data.shape
    data = data.reshape((-1, chans, bands))
    data = data.reshape((videos * 30, 256, 32, 4))
    features = np.zeros((videos * 30, 9, chans, bands))
    step_size = 20
    window_size = 96
    for seg in range(videos * 30):
        for chan in range(chans):
            for band in range(bands):
                trial = data[seg, :, chan, band]
                for i in range(9):
                    DEfeature = compute_DE(trial[i * step_size:i * step_size + window_size])
                    # normed = DEfeature - baseline[seg//30, chan, band]
                    features[seg, i, chan, band] = DEfeature
    return features
