...
class GFK:
    ...
    def train_pca(self, data, mu_data, std_data, subspace_dim):
        ...
        # data is the normalised data
        if isinstance(subspace_dim, float):
            # subspace_dim as float is treated as the desired explained variance ratio
            pca = PCA(n_components=subspace_dim)
        else:
            # subspace_dim as int is treated as the desired number of components
            pca = PCA(n_components=int(subspace_dim))

        # Fit PCA on the normalized data
        pca.fit(data)

        # No direct equivalent of machine.resize(), but n_components handles dimensionality reduction
        # The PCA object can be used to transform data to the lower-dimensional space
