import argparse
import os
from model_dural_branch import MultiBandDataset, FusionModel, train, evaluate
...
config_file = 'configs/DEAP/cross10.conf'
...
config_dict = parse_conf_file(config_file)
wandb.init(
    # set the wandb project where this run will be logged
    project="watch_deap_cross10",
...
constructed = construct_graphs(args.data, args.dataset, args.window_length)
constructed_train, constructed_test = split_data(constructed, test_ratio=0.1, random_flag=False)
...
for epoch in tqdm.tqdm(range(num_epochs)):
    if wait >= args.patience:
        ...
        break
    train(model, tr_loader, optimizer, criterion, device, args.max_grad_norm)
