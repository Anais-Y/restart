...
bands = ["alpha", "beta", "gamma", "theta", "de"]
person = '15_20131105'
for band in bands:
    edge_index_np = None
    ...
    for n in range(1123):
        ...
        # else:
            # attention_weights_np += attention_weights.cpu().numpy().squeeze()  # 累加attention_weights
        ...
        np.save(f'/data/Anaiis/garage/adj_matrix/{person}-{band}-adj_matrix{n}-{label}.npy', adj_matrix)
...
# plt.show()
# plt.savefig(f'electron/fig-label{selec_lab}-{person}-{band}')