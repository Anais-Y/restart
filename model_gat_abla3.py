...
class FusionModel(nn.Module):
    def __init__(self, num_node_features, hidden_dim, num_heads, dropout_disac, num_classes, dataset):
        ...
        self.GAT = GAT(num_node_features=num_node_features, hidden_dim=hidden_dim, num_heads=num_heads,
    ...
    def forward(self, data):
        x_alpha = self.GAT(data['alpha'])
        x_beta = self.GAT(data['beta'])
        x_gamma = self.GAT(data['theta'])
        x_theta = self.GAT(data['gamma'])
        if self.dataset == "DEAP":
            x_de = self.GAT_de(data['de'])
            x_concat = torch.cat((x_alpha, x_beta, x_gamma, x_theta, x_de), dim=1)
        elif self.dataset == "SEED":
            x_delta = self.GAT(data['delta'])
            x_de = self.GAT_de(data['de'])
