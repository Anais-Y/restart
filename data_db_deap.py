...
def compute_features_96(filtered_data, baseline):  # , baseline  num_windows: 每s要分成多少个windows
    data = filtered_data[:, :, 3 * 128:, :].transpose((0, 2, 1, 3))  # 40 7680 32 4
    videos, t, chans, bands = data.shape
    # data = data.reshape((-1, chans, bands))
    # data = data.reshape((videos * 30, 256, 32, 4))
    features = np.zeros((videos, t // 96, chans, bands))
    # step_size = 20
    # window_size = 96
    for v in range(videos):
        for chan in range(chans):
            for band in range(bands):
                for i in range(80):
                    trial = data[v, i*96: i*96+96, chan, band]
